...
function isPalindrome(str) {
  const clearedString = str.toLowerCase().replace(/[^a-z]/g, '');
...
function findLongestWord(sentence) {
  ...
  const longestWordIndex = words.reduce((longestIndex, currentWord, index) => {
    if (currentWord.length > words[longestIndex].length) {
      return index;
    }
    return longestIndex;
  }, 0);
...
function encodeToRot13(str) {
  ...
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    result += shiftedTable[table.indexOf(str[i])];
  }
  return result;
}
